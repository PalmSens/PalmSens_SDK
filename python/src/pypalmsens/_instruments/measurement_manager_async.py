...
class MeasurementManagerAsync:
    ...
    def setup_handlers(self):
        self.begin_measurement_handler = AsyncEventHandler[
            CommManager.BeginMeasurementEventArgsAsync
        ](self.begin_measurement_callback)

        self.end_measurement_handler = AsyncEventHandler[
    ...
    def curve_data_added_callback(self, curve: PSCurve, args):
        ...
        future = asyncio.run_coroutine_threadsafe(
            self.curve_new_data_added_coroutine(curve, start, count), self.loop
        )
        # block c# core library thread to apply backpressure and prevent unnescessary load on the python asyncio eventloop
        future.result()

    async def curve_new_data_added_coroutine(self, curve: PSCurve, start: int, count: int):
        self.curve_new_data_added(curve, start, count)
