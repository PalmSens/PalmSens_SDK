...
: Shows how to connect to a collection of instruments and run a
chronopotentiometry measurement on all channels simultaneously using hardware
synchronization.
...
- Python dependencies listed `requirements.txt`
  - Either run the following command: `pip install -r requirements.txt` or refer to the steps in The next chapter
- .NET Framework 4.7.2
- Drivers included with *PSTrace5.x*, *MultiTrace4.x*, *PSTrace Xpress* or the
  included driver installer


> **Note**: Python 3.13 contains [a known issue](https://github.com/python/cpython/issues/130522) which will print errors to the output due to threads not being disposed correctly, these errors can safely be ignored. `Exception ignored in: <function _DeleteDummyThreadOnDel.__del__ at 0x000001B73AEEE5C0`


### [Optional] Create a virtual environment in Visual Studio Code

[Visual studio code](https://code.visualstudio.com/) in combination with the Python, Python Debugger and Pylance extensions offer an easy command to create a virtual environment and load the python dependencies.

1. Press <kbd>control</kbd>+<kbd>shift</kbd>+<kbd>p</kbd> to open the command pallete and select the `Python: Create Environment command to create a new virtual environment.`

![Create virtual environment in VSCode](../assets/vscode_create_venv.png)

2. Select the `Venv` option

![Select virtual environment in VSCode](../assets/vscode_select_venv.png)

3. Select an installed python interpreter

![Select Python interpreter in VSCode](../assets/vscode_select_python.png)

4. Select the `requirements.txt` to install the python dependencies for the
pspython SDK module and examples.

![Select dependencies in VSCode](../assets/vscode_select_dependencies.png)

## Working with files

As of version 5 of the PalmSens SDK and PSTrace measurements and their
corresponding methods are stored in `*.pssession` files. Methods can
be stored separately in `*.psmethod` files.
...
The `pspyfiles.py` script function `load_method_file()` can be used to load
method files. 
This function returns a `PalmSens.Method` .NET object which
can be used to run a measurement.
...
The `pspymethods.py` script contains helper functions to create .NET method
objects for the following techniques:
...
technique. 
Parameters of a .NET method object can be modified by
...
functions in `pspymethods.py` you need to import the .NET class, create an
instance and adjust the parameters. Appendix A lists the classes for the
techniques.
...
> **Tip**: The VSCode Debug Console or another Python REPL environment will
auto complete on the properties and functions of .NET objects like
the `PalmSens.Method` objects returned by the helper methods. 
...
![Data in pssession files](../assets/pssession.png)

Data from measurements can be loaded from
and stored to `.pssession` files. 
This contains a session with one
or more measurements containing its respective method and curves.
...
Currently the `pspython` module supports discovering instruments connected via FTDI, serial (usbcdc/com), and Bluetooth (classic/low energy). 
By default scanning with Bluetooth is disabled.
...
The InstrumentManager measure function returns a `Measurement` and also supports keeping a reference to the underlying .NET object for more information please refer to Chapter
3.4.
...
- **Non-impedimetric techniques**. Techniques such as linear sweep voltammetry or chronopotentiometry return a dictionary containing the following values:
...
- **Impedimetric techniques**. the exception are (galvanostatic/) electrochemical impedance spectroscopy. These techniques return the following:
...
> **Mains Frequency**: To eliminate noise induced by other electrical appliances it is highly recommended to set your regional mains frequency (50/60 Hz) in the static property `PalmSens.Method.PowerFreq`.

### MethodSCRIPT&trade;

The MethodSCRIPT&trade; scripting language is designed to integrate our OEM potentiostat (modules) effortlessly in your hardware setup or product.

MethodSCRIPT&trade; allows developers to program a human-readable script directly into the potentiostat module by means of a serial (TTL) connection.
...
See the [MethodSCRIPT&trade; documentation](https://www.palmsens.com/methodscript) for more information.

#### Sandbox Measurements

PSTrace includes an option to make use MethodSCRIPT&trade; Sandbox to write and run scripts.
This is a great place to test MethodSCRIPT&trade; measurements to see what the result would be.
That script can then be used in the MethodScriptSandbox technique in the SDK as demonstrated
below.


![Graphical editor for MethodSCRIPT&trade;](../assets/method_script_editor.png)
...
The measurement class contains all classes, functions, and parameters necessary for performing a measurement with a PalmSens or EmStat instrument. 
...
![Layout for Measurement class](../assets/sdk_measurement_class.png)

The following diagram shows the inheritance structure of the `Method`
classes:

![Inheritance structure for Method classes](../assets/sdk_method_inheritance.png)


## Appendix A: Parameters for each technique

All applicable parameters for each technique can be found here.
For the inheritance hierarchy of the the techniques, see section 3 in this document.
...
### Common properties
  ...
  **Property**             **Description**                 **Type**                 
  ------------------------ ------------------------------- -------------------------
  `Technique`              The technique number used in    `System.Int`             
                           the firmware                                             
  
  `Notes`                  Some user notes for use with    `System.String`          
                           this method                                              
  
  `StandbyPotential`       Standby Potential (for use      `System.Float`           
                           with cell on after                                       
                           measurement)                                             
  
  `StandbyTime`            Standby time (for use with      `System.Float`           
                           cell on after measurement)                               
  
  `CellOnAfterMeasurement` Enable/disable cell after       `System.Boolean`         
                           measurement                                              
  
  `MinPeakHeight`          Determines the minimum peak     `System.Float`           
                           height in µA. Peaks lower                                
                           than this value are                                      
                           neglected.                                               
  
  `MinPeakWidth`           The minimum peak width, in      `System.Float`           
                           the unit of the curves X                                 
                           axis. Peaks narrower than                                
                           this value are neglected.                                
  
  `SmoothLevel`            The smoothlevel to be used.     `System.Int`             
                           `-1` = none                                              
                           0 = no smooth (spike                                     
                           rejection only)                                          
                           `1` = 5 points                                           
  ...
  `Ranging`                Ranging information, settings   `PalmSens.Method.Ranging`
                           defining the                                             
                           minimum/maximum/starting                                 
                           current range                                            
  
  `PowerFreq`              Adjusts sampling on             `System.Int`             
                           instrument to account for                                
                           mains frequency. It accepts                              
                           two values:                                              
                           50 for 50Hz                                              
                           60 for 60Hz                                              
  ----------------------------------------------------------------------------------
...
The following properties specify the measurements pretreatment settings:
  ...
  **Property**            **Description**                              **Type**
  ...
  EquilibrationTime       Equilibration duration in seconds.           `System.Float`
                          BeginPotential is applied during             
                          equilibration and the device switches to the 
...
Class: `Palmsens.Techniques.LinearSweep`
  ...
  **Property**     **Description**                                  **Type**
  ...
  Scanrate         The applied scan rate. The applicable range      `System.Float`
                   depends on the value of E step since the data    
                   acquisition rate is limited by the connected     
                   instrument.                                      
  --------------------------------------------------------------------------------
...
Class: `Palmsens.Techniques.SquareWave`
  ...
  **Property**     **Description**                               **Type**
  ...
  PulseAmplitude   Amplitude of square wave pulse. Values are    `System.Float`
                   half peak-to-peak.                            
...
Class: `Palmsens.Techniques.ACVoltammetry`
  ...
  **Property**         **Description**                      **Type**
  ...
  SineWaveAmplitude    Amplitude of sine wave. Values are   `System.Float`
                       RMS                                  
...
Outdated class. PalmSens 3 and 4 only.
For CV's with sampling over 5000 data points per second, use the regular `Palmsens.Techniques.CyclicVoltammetry()` constructor instead.
...
`MeasurementTime`     The maximum measurement time. This value   `System.Float` 
                      should always exceed the required                      
                      measurement time. It only limits the                   
                      time of the measurement. When the                      
                      potential response is erroneously and E                
                      end is not found within this time, the                 
                      measurement is aborted.                                

`AppliedCurrentRange` The applied current range                  `PalmSens.CurrentRange` 

`Istrip`              If specified as 0, the method is called    `System.Float` 
                      chemical stripping otherwise it is                     
                      constant current stripping. The current                
                      is expressed in the applied current                    
                      range.                                                 
--------------------------------------------------------------------------------
...
Class: `PalmSens.Techniques.PulsedAmpDetection`
  ...
  **Property**         **Description**                         **Type**              
  ...
  `PulsePotentialAD`   Potential in pulse. Note that this      `System.Float`          
                       value is not relative to dc/base                              
                       potential, given above.                                       
  
  `PulseTime`          The pulse time.                         `System.Float`          
  
  `tMode`              *DC*: I(dc) measurement is performed at   `PalmSens.Techniques.PulsedAmpDetection.enumMode`
                       potential E

                       *pulse*: I(pulse) measurement is                                
                       performed at potential E pulse  

                       *differential*: I(dif) measurement is                           
                       I(pulse) - I(dc)                                              
...
Class: `PalmSens.Techniques.FastAmperometry`
  ...
  **Property**      **Description**                       **Type**
  ----------------- ------------------------------------- ---------------
  `EqPotentialFA`   Equilibration potential at which the  `System.Float`
                    measurement starts.                   
...
**Property**          **Description**                           **Type**     
--------------------- ----------------------------------------- -----------------
`Current`             The current to apply. The unit of the     `System.Float` 
                      value is the applied current range. So                 
                      if 10 uA is the applied current range                  
                      and 1.5 is given as value, the applied                 
                      current will be 15 uA.                                 
...
Class: `PalmSens.Techniques.MultiplePulseAmperometry`
  ...
  **Property**   **Description**                                 **Type**
  -------------- ----------------------------------------------- --------------
  `E1`           First potential level in which the current is   `System.Float`
                 recorded                                        
...
`enumScanType.FixedPotential` and `FreqType = ImpedimetricMethod.enumFrequencyType.Scan`). The additional
...
separately in next sections.
  ...
  **Property**       **Description**                          **Type**             
  ------------------ ---------------------------------------- ------------------------
  `ScanType`         Scan type specifies whether a single     `ImpedimetricMethod.enumScanType` 
                     or multiple frequency scans are          
                     performed. When set to FixedPotential                         
                     a single scan will be performed, this                         
                     is the recommended setting. **The                             
                     TimeScan and PotentialScan are not                            
                     fully supported in the SDK**, we                              
                     highly recommend you to implement                             
                     yourself. A TimeScan performs repeated                        
                     scans at a given time interval within                         
                     a specified time range. A                                     
                     PotentialScan performs scans where the                        
                     DC Potential of the applied sine is                           
                     incremented within a specified range.                         
                     A PotentialScan should not be                                 
                     performed versus the OCP.                                     

  `Potential`        The DC potential of the applied sine     `System.Float`         

  `Eac`              The amplitude of the applied sine in     `System.Float`         
                     RMS (Root Mean Square)                                        

  `FreqType`         Frequency type specifies whether to      `ImpedimetricMethod.enumFrequencyType`
                     perform a scan on a range of                 
                     frequencies or to measure a single                            
                     frequency. The latter option can be                           
                     used in combination with a TimeScan or                        
                     a PotentialScan.                                              

  `MaxFrequency`     The highest frequency in the scan,       `System.Float`         
                     also the frequency at which the                               
                     measurement is started                                        

  `MinFrequency`     The lowest frequency in the scan         `System.Float`         

  `nFrequencies`     The number of frequencies included in    `System.Int`           
                     the scan                                                      

  `SamplingTime`     Each measurement point of the            `System.Float`         
                     impedance spectrum is performed during                        
                     the period specified by SamplingTime.                         
                     This means that the number of measured                        
                     sine waves is equal to SamplingTime \*                        
                     frequency. If this value is less than                         
                     1 sine wave, the sampling is extended                         
                     to 1 / frequency. So for a measurement                        
                     at a frequency, at least one complete                         
                     sine wave is measured.                                        
                                                                                   
                     Reasonable values for the sampling are                        
                     in the range of 0.1 to 1 s.                                   

  `MaxEqTime`        The impedance measurement requires a     `System.Float`         
                     stationary state. This means that                             
                     before the actual measurement starts,                         
                     the sine wave is applied during                               
                     MaxEqTime only to reach the stationary                        
                     state.                                                        
                                                                                   
                     The maximum number of equilibration                           
                     sine waves is however 5. The minimum                          
                     number of equilibration sines is set                          
                     to 1, but for very low frequencies,                           
                     this time is limited by MaxEqTime. The                        
                     maximum time to wait for stationary                           
                     state is determined by the value of                           
                     this parameter. A reasonable value                            
                     might be 5 seconds. In this case this                         
                     parameter is only relevant when the                           
                     lowest frequency is less than 1/5 s                          
                     so 0.2 Hz.                                                    
------------------------------------------------------------------------------------
...
for this that suits your demands.
  ...
  **Property**       **Description**                             **Type**
  ------------------ ------------------------------------------- --------------
  `RunTime`          RunTime is not the total time of the        `System.Float`
                     measurement, but the time in which a        
                     measurement iteration can be started. If a  
                     frequency scan takes 18 seconds and is      
                     measured at an interval of 19 seconds for a 
                     RunTime of 40 seconds three iterations will 
                     be performed.                               

  `IntervalTime`     IntervalTime specifies the interval at      `System.Float`
                     which a measurement iteration should be     
                     performed, however if a measurement         
                     iteration takes longer than the interval    
                     time the next measurement will not be       
                     triggered until after it has been           
                     completed.                                  
  -----------------------------------------------------------------------------
...
this that suits your demands.
  ...
  **Property**         **Description**                             **Type**
  -------------------- ------------------------------------------- --------------
  `BeginPotential`     The DC potential of the applied sine wave   `System.Float`
                       to start the series of iterative            
                       measurements at.                            

  `EndPotential`       The DC potential of the applied sine wave   `System.Float`
                       at which the series of iterative            
                       measurements ends.                          

  `StepPotential`      The size of DC potential step to iterate    `System.Float`
                       with.                                       
  -------------------------------------------------------------------------------
...
- AuxInput, similar to PSTrace it is possible to configure the readings
  of the auxilliary input. Using the
  `PalmSens.AuxInput.AuxiliaryInput` class you can assign a name,
  offset, gain and unit to the auxilliary input. The following example
  demonstrates how to set up the Pt1000 temperature sensor from PSTrace.

  ![Change auxilary input](../assets/auxilary_input.png)
...
The multiplexer functionality is demonstrated in the PSSDKMultiplexerExample project.
...
This mode can be set for all techniques but Electrochemical Impedance Spectroscopy.
