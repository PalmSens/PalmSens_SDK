...
3. Rename `MeasureApp.csproj` to `MyPalmSensApp.csproj`
4. Right click menu:Solution[Add > Existing Project], select `MyPalmSensApp.csproj` file
5. Update namespace (find-replace), `MeasureApp` -> `MyPalmSensApp`
...
This is a basic example that shows how to get started with building a simple data measurement app using the PalmSens .NET MAUI SDK.
It shows how to discover devices, make a connection, run a measurement, and show the data in a table.
